...
models = [
    ...
    # SentenceTransformer('nli-roberta-base-v2'), # 56 too. mismatched "what are my best accounts" to "List my accounts with the most whitespace?". Whitespace does not factor in any other criteria like account fit. Though this could arguably be okay given the lack of context. Maybe we should look into this a bit.
...
def get_combined_embeddings(models, texts: [str]) -> np.array:
    embeddings_list = [get_embeddings(model, texts) for model in models]
...
def match_questions(questions: [str], user_input: str, top_n=3, threshold=0.80) -> list:
    combined_question_embeddings = get_combined_embeddings(models, questions)
    combined_user_input_embed = get_combined_embeddings(models, [user_input])[0]

    # Compute cosine similarity between the user input embedding and the combined question embeddings.
    similarities = [util.pytorch_cos_sim(combined_user_input_embed, question_embed).item() for question_embed in
